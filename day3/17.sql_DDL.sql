...
/*
    ...
    -- 타입 종류
    INT[정수], BIGINT, BINARY(50)[이진데이터], BIT[0|1], CHAR(n)[고정문자열], VARCHAR(n)[가변문자열], NCAHR(n), NVACHAR(n) N - [유니코드]
    예(username: CHAR(20)/VARCHAR(10)) 'hugo'
    CAHR(10) = 'hugo    ' 이름을 넣고 10개의 자리중 빈곳은 전부 스페이스 입력 ' '
    VARCHAR(10) = 'hugo' 이름을 넣고 10개의 자리중 빈곳은 전부 없앰

    DATE[날짜], DATETIME[날짜시간],DECIMAL(18, 0)[소수점표현실수], FLOAT[실수], IMAGE[이미지바이너리], SMALLINT[255까지의 정수], TEXT[2GB까지의 텍스트], NTEXT[유니코드 26]
...
CREATE TABLE NewBook (
    ...
    price INT
    PRIMARY KEY (bookid) -- 기본키 지정
);

-- 기본키를 통합
CREATE TABLE NewBook (
    bookid INT PRIMARY KEY, -- 기본키지정
    bookname VARCHAR(20),
    publisher VARCHAR(20),
    price INT
