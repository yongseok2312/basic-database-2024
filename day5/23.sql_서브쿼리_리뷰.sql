-- where절 서브쿼리 리뷰
-- ALL, ANY(SOME) -ANY랑 SOME을 쓰는 조건은 아래의 데이터에서는 보기 힘듬
...
-- SELECT절 서브쿼리, JOIN으로 변경 가능 이미 쿼리가 너무 복잡해서 더이상 테이블을 추가하기 힘들면, 많이 사용
-- 고객별 판매액을 보이시오
SELECT *
    FROM Orders AS o
    GROUP BY o.custid
...
-- GROUP BY에 들어갈 컬럼은 최소화 해야함, 중복등의 문제가 있으면 결과가 다 틀어짐
SELECT SUM(o.saleprice) AS '고객별 판매액'
