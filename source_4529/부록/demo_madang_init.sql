-- 실습에 필요한 Madang DB 를 초기화 합니다.

-- Book, Customer, Orders 테이블 및 자료 초기화
USE [Madang]


IF EXISTS(SELECT NAME FROM sys.all_objects WHERE TYPE_DESC='USER_TABLE' AND (NAME='orders' or name='Orders'))
     DROP TABLE [Orders];

IF EXISTS(SELECT NAME FROM sys.all_objects WHERE TYPE_DESC='USER_TABLE' AND (NAME='customer' or name='Customer'))
     DROP TABLE [Customer];

IF EXISTS(SELECT NAME FROM sys.all_objects WHERE TYPE_DESC='USER_TABLE' AND (NAME='book' or name='Book'))
     DROP TABLE [Book];
